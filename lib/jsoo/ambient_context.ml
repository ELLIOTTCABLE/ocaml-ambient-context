...
let debug () = Jsoo.Js.Unsafe.global ##. OCAML_AMBIENT_CONTEXT_DEBUG
let debug_id = ref 0
...
let compare_key = ( - )
let default_storage = Ambient_context_JSOO_globals.storage ()
let current_storage = ref default_storage
let get_current_storage () = !current_storage
...
let set_storage_provider store_new =
   ...
   if store_new == store_before then () else current_storage := store_new ;
   if debug () then
